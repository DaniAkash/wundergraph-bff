...
const Home: NextPage = () => {
  const launchPads = useQuery({
    operationName: "launchPadsWithWeather",
    liveQuery: true,
  });
      ...
      <div className="relative flex flex-col items-center overflow-hidden p-8">
        <div className="w-full max-w-xl rounded-2xl bg-white px-20 py-12">
          <div className="flex flex-col gap-10">
            {launchPads.data?.spacex_launchpads?.map((each) => {
              return (
                <LaunchPadDetails
                  name={each.location?.name!}
                  region={each.location?.region!}
                  successful_launches={each.successful_launches!}
                  attempted_launches={each.attempted_launches!}
                  weather={
                    each._join.weather_getCityByName?.weather?.summary
                      ?.description!
                  }
                />
              );
